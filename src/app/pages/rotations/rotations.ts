...
export class Rotations {
  ...
  subtitle(e: Entity): string {
    ...
    if (e.special) return e.special.kind === 'bomb' ? 'thrown · no GCD' : e.special.kind === 'device' ? 'deployed · no GCD' : e.special.adrenaline || e.special.adrenalineOverTime ? '+' + (e.special.adrenaline || e.special.adrenalineOverTime) + '% adrenaline' : 'potion · no GCD';
